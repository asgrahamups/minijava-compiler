...
/**
 * A pretty-printer for IRT code. The IRT node classes aren't set up to allow
 * visitors, so we traverse the tree the "old fashioned" way: Use instanceof to
 * figure out which Stm or Exp we're dealing with, and jump to a method that
 * knows how to print the particular node. In general, you'll use Print.prStm()
 * to print an IRT program. If you've got an IRT fragment whose root is an Exp
 * node, you'll need to use Print.prExp().
 */

public class Print {

	/**
	 * Top-level call to start the traversal and printing of an IRT tree whose
	 * root node is a statement.
	 * 
	 * @param s
	 *            The root (statement node) of an IRT tree
	 */
	...
	/**
	 * Top-level call to start the traversal and printing of an IRT tree whose
	 * root node is an expression.
	 * 
	 * @param e
	 *            The root (expression node) of an IRT tree
	 */
